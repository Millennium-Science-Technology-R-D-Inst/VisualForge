...
export module Core.AppSettingsDatabase;
...
export namespace Core
{
	...
	public:
		...
		void SetInt(std::string const& category, std::string const& key, std::int64_t value);

		/// Get an integer setting
		std::optional<std::int64_t> GetInt(std::string const& category, std::string const& key) const;

		/// Get an integer setting with default value
		std::int64_t GetInt(std::string const& category, std::string const& key, std::int64_t defaultValue) const
		{
